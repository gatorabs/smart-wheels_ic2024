...
DISTANCIA_CENTRO_FAIXA = 100 # distancia centro do veiculo / camera até a faixa, para controle
KP = 0.3
KI = 0.005
KD = 0.01
MIN = -52
...
PORTA_COM    = 'COM5'  # porta COM para comunicar com Arduino
porta_serial = True
...
enviar_dados = [255, 180, 0, 0, 0, 0, 0, 0, 0]
tempo_anterior = time.time()


# **********************************#
# Funcao: Class PID
# Proposito: classe de controle
# Obs:
# **********************************#
class pid_direcao:
    # calibrado para os sensores
    kp = 0.0
    ki = 0.0
    kd = 0.0
...
cv.createTrackbar('canny_1', 'Controles', 20, 300, nothing)
cv.createTrackbar('canny_2', 'Controles', 60, 400, nothing)
cv.createTrackbar('velocidade', 'Controles', 0, 255, nothing)


# **********************************#
# Funcao: enviar_dados_serial()
# Proposito: enviar dados serial a cada 0.1 segundos
# Obs:
# **********************************#
...
# Proposito: fechar programa jumto com as conexoes
# Obs:
# **********************************#
...
if ENVIAR_DADOS == True:
    ...
    porta_serial = serial.Serial(PORTA_COM, 115200)
    time.sleep(1)
...
while(1):
    ...
    enviar_dados[0] = cv.getTrackbarPos('velocidade', 'Controles')
